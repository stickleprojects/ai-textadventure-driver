# Requirements

Agent gameplay capabilities — things the agent must be able to do to play Knight Orc effectively.

| ID  | File                          | Title                                               | Status  |
| --- | ----------------------------- | --------------------------------------------------- | ------- |
| 2   | [2.md](requirements/2.md)     | NPC greeting and quest tracking                     | 🔲 Open  |
| 6   | [6.md](requirements/6.md)     | Agent cannot wear items or disguises                | ✅ Fixed |
| 10a | [10a.md](requirements/10a.md) | Death and end-state detection                       | ✅ Fixed |
| 10b | [10b.md](requirements/10b.md) | Score parsing                                       | ✅ Fixed |
| 10c | [10c.md](requirements/10c.md) | Cross-run learning                                  | 🔲 Open  |
| 18  | [18.md](requirements/18.md)   | Fixed inspection sequence for all objects           | ✅ Fixed |
| 20  | [20.md](requirements/20.md)   | No mechanism for user strategy hints                | ✅ Fixed |
| 21  | [21.md](requirements/21.md)   | Cannot execute timed multi-step NPC strategies      | 🔲 Open  |
| 22  | [22.md](requirements/22.md)   | No detection of NPC movement events                 | 🔲 Open  |
| 23  | [23.md](requirements/23.md)   | No detection of NPC inventory transfers             | ✅ Fixed |
| 25  | [25.md](requirements/25.md)   | No response to route blockages                      | ✅ Fixed |
| 27  | [27.md](requirements/27.md)   | Verb failures re-learned from scratch each run      | ✅ Fixed |
| 40  | [40.md](requirements/40.md)   | No handling of NPC-initiated dialogue               | 🔲 Open  |
| 43  | [43.md](requirements/43.md)   | Hint system limited to inventory-solution anomalies | 🔲 Open  |
| 44  | [44 ui matrix.md](requirements/44%20ui%20matrix.md) | map features in the ui                              | 🔲 Open  |
| 45  | [45.md](requirements/45.md)   | Agent doesn't interrupt current activity for urgent events | 🔲 Open  |

